...
#This can be changed to easily do social security numbers or othe items with similiar formats
...
#$var_group = get-childitem -Recurse -File | ?{$_.extension -EQ ".txt"} | %(write-host $_.name)
$var_group = (get-childitem -Recurse -file).Name
...
#if the expression is matched, the return is true.
#Added toupper() to this so that expressions such as PASSWORD could be searched by changing the var_match_exp expression.
foreach($item in $var_group) {
 ...
 $var_result = $var_filetest.ToUpper() -match $var_match_exp
    if ($var_result -eq $true)
        {write-host $item "contains possbile cc data" }
}